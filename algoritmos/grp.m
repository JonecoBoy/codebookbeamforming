...
for n=1:1:NG
   ...
   w_ajuste = autovetores * autovalores^1/2 * quisi(n);%
    w(:,n) = w_ajuste(:,1)
    wtil(:,n) = 1/sqrt(L) *exp(j*angle(w(:,n)));
end

for i=1:1:size(max_wtil)
[argmax_wtil(i) ,indice(i)] = max(wtil(i,:))
wtil_0(i) = wtil(indice(i))
end
wtil = wtil' %por definicao o de cada patern e vertical
