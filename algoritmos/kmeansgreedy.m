...
x = [-1:espacamento:1];     
theta= acos(x);             
q=3;                        % controla a direcao /ganho do feixo formado, quanto maior mais estreito o feixe
...
arrayFactorKmeansTotal(jj,i) = max(arrayFactorKmeans(jj,:,i));
 ...
 i=1;   
Umean_vetor(jj) = Umean;
 txt = ['Umean: ' num2str(Umean)];
 if jj > 1
    if Umean_vetor(jj) <= Umean_vetor(jj-1)
        controlegreedy = 1
        Umean_otimo =  Umean_vetor(jj-1)
    end
 end
jj = jj+1
end
...
stringPlot={'b','r','g','c','m','y','k','b--','r--','g--','c--', 'm--','y--','k--','b','r'};
figure;
 for i=1:size(arrayFactorKmeansmax,1)
     polarplot(theta,abs(arrayFactorKmeansmax(i,:)),stringPlot{i});
     title(['\fontsize{16} CodeBook - Kmeans K =',num2str(K),' L =',num2str(L)]);
     hold on;
...
hold on
for k=1:K
plot(rad2deg(theta-pi),abs(arrayFactorKmeansmax(k,:))',stringPlot{k})
...
y=0;
for i = 0:1:100
y(length(y)+1) = prctile(abs(arrayFactorKmeansTotalmax),i);
end
y(1)=[];
plot(y,0:1:100,stringPlot{k});
hold on;
axis([0.85 1 0 100])
